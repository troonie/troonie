...
#region Public Methods
		...
		internal void PIC_ENHANCE_Clear()
		{
			if (exif_ifd != null)
				exif_ifd.PIC_ENHANCE_Clear ();

			if (gps_ifd != null)
				gps_ifd.PIC_ENHANCE_Clear ();

			if (Structure != null)
				Structure.PIC_ENHANCE_Clear ();
		}
		...
		public override void CopyTo (Tag target, bool overwrite)
		{
			...
			IFDTag t = target as IFDTag;

			if (overwrite || IsNullOrLikeEmpty (t.DateTimeOriginal))
				t.DateTimeOriginal = DateTimeOriginal;

			if (overwrite || IsNullOrLikeEmpty (t.DateTimeDigitized))
				t.DateTimeDigitized = DateTimeDigitized;
